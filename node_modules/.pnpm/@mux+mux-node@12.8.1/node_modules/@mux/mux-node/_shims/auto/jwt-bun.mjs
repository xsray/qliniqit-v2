...
import fs from 'fs';
import { isKeyLike, keyFormatErrorMessage, unwrapPem, pkcs1to8, toPkcs8Pem } from "../../util/jwt-util.mjs";
...
async function getPrivateKeyHelper(mux, opts) {
    ...
    if (Buffer.isBuffer(key) || isKeyLike(key))
        return key;
    if (typeof key === 'string') {
        key = key.trim();
...
//# sourceMappingURL=jwt-bun.mjs.map