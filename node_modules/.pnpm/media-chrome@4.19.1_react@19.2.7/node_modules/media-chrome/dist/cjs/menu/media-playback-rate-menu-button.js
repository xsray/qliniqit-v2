...
function getSlotTemplateHTML(attrs) {
  const rate = attrs["mediaplaybackrate"] ? (0, import_media_playback_rate_button.normalizePlaybackRate)(+attrs["mediaplaybackrate"]) : DEFAULT_RATE;
  return (
    /*html*/
    `
    <style>
...
class MediaPlaybackRateMenuButton extends import_media_chrome_menu_button.MediaChromeMenuButton {
  ...
  constructor() {
    ...
    this.container.innerHTML = `${(0, import_media_playback_rate_button.normalizePlaybackRate)((_a = this.mediaPlaybackRate) != null ? _a : DEFAULT_RATE)}x`;
  }
  attributeChangedCallback(attrName, oldValue, newValue) {
    super.attributeChangedCallback(attrName, oldValue, newValue);
    if (attrName === import_constants.MediaUIAttributes.MEDIA_PLAYBACK_RATE) {
      const newPlaybackRate = newValue ? +newValue : Number.NaN;
      const playbackRate = (0, import_media_playback_rate_button.normalizePlaybackRate)(
